...
Deno.test("GET /api/config: exposes agent_hour_utc/agent_hour_utc_2/agent_daily_picks from valid vars", async () => {
  const env = makeEnv({ AGENT_HOUR_UTC: "5", AGENT_HOUR_UTC_2: "17", AGENT_DAILY_PICKS: "12" });
  const res = await app.request("/api/config", {}, env, makeExecutionContext());
  assertEquals(res.status, 200);
  const body = await res.json();
  assertEquals(body.agent_hour_utc, 5);
  assertEquals(body.agent_hour_utc_2, 17);
...
Deno.test("GET /api/config: agent_hour_utc_2 is null when unset/empty/invalid (independent of AGENT_HOUR_UTC)", async () => {
  const unset = makeEnv({ AGENT_HOUR_UTC: "5" });
  const resUnset = await app.request("/api/config", {}, unset, makeExecutionContext());
  const bodyUnset = await resUnset.json();
  assertEquals(bodyUnset.agent_hour_utc_2, null);
  assertEquals(bodyUnset.agent_hour_utc, 5);

  const invalid = makeEnv({ AGENT_HOUR_UTC: "5", AGENT_HOUR_UTC_2: "24" });
  const resInvalid = await app.request("/api/config", {}, invalid, makeExecutionContext());
  assertEquals((await resInvalid.json()).agent_hour_utc_2, null);
});
